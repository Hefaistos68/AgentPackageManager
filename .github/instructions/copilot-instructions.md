---
applyTo: '**'
description: 'General instructions for the C# Expert Agent, including core directives, interaction philosophy, code generation guidelines, and NUnit best practices.'
---
...
- C# Expert Agent rules: [..\agents\CSharpExpert.agent.md](..\agents\CSharpExpert.agent.md)
- C# Documentation Prompt: [.\csharp-docs.md](.\csharp-docs.md)
...
-   **Minimize feedback wording on process**: Instead of saying "I will now use tool X to find Y", say "To find Y, I will use tool X". Focus on the action and its purpose rather than the process of deciding to use the tool. Dont give yourself personality in the response. Instead of saying "I'll check ..." say "Checking ...". Avoid phrases that anthropomorphize your actions.
...
- For unit tests always use NUnit, Moq and Shouldly. Create a new object of the class to test for each test, dont use a test class member.
- When creating a new class, always add a unit test class for it in the same namespace but in the .Tests project.
- When adding new classes, interfaces or methods, always add XML documentation comments to all members of the new code, including parameters and return types, following the best practices for C# documentation.
...
- Reference `Microsoft.NET.Test.Sdk`, `NUnit`, and `NUnit*TestAdapter` packages
